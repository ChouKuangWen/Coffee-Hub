...
| 項目 | 詳細說明 | 目標價值 |
|------|-----------|-----------|
| **6. CI/CD 自動化** | 建立 `GitHub Actions` 或 `GitLab CI` 流水線，實現程式碼合併後的自動測試、建置 `Docker` 映像檔和自動部署。 | 提升開發效率、縮短交付週期、保證部署品質。 |
| **7. Log / 錯誤追蹤** | 整合 Sentry 或類似的錯誤監控服務，以便即時追蹤、警報和診斷生產環境中的運行錯誤。 | 快速定位問題，提升系統穩定性和維護效率。 |
