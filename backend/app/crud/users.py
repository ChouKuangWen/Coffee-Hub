...
async def update_user_crud(db: AsyncSession, user_id: int, user_update: UserUpdate) -> Optional[Users]:
    ...
    for key, value in update_data.items():
        # 檢查是否為需要淨化的字串欄位
            setattr(db_user, key,  value) # 使用淨化後的值

    await db.flush()    # 提交資料庫變更
    return db_user

# 非同步刪除使用者
async def delete_user_crud(db: AsyncSession, user_id: int) -> Optional[Users]:
    db_user = await get_user_by_id(db, user_id)
    if not db_user:
        return None         # 找不到使用者回傳 None
    await db.delete(db_user)  # 刪除 ORM 物件
    await db.flush()        # 提交變更
    return db_user             # 刪除成功回傳
